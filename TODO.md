...
- [x] **Simulated fills (Rails runner / `ExecutionEngine`):** opt-in **`PAPER_USE_ORDERBOOK_SIMULATOR`** — shared `PaperTrading::DeltaLikeFillSimulator`, multi-fill `FillProcessor`, fees; default remains instant fill for backward compatibility. Strict limit / empty book: **`PAPER_LIMIT_FILL_STRICT`**. Portfolio balance debits **per-fill fee** via `Portfolio#apply_fill_and_sync!`.
...
- [x] **P1 — Unified paper execution simulator:** `PaperTrading::DeltaLikeFillSimulator`; `PAPER_USE_ORDERBOOK_SIMULATOR` + `PAPER_LIMIT_FILL_STRICT` on `Trading::ExecutionEngine`; `Portfolio#apply_fill_and_sync!` fee debit; docs (`paper_vs_live_parity`, `README`, `CHANGELOG`, `.env.example`).
