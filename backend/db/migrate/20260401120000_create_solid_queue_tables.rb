...
# Solid Queue tables for development/test: production may use a separate DB (see config/database.yml
# `queue` + db/queue_schema.rb). Local ./bin/dev runs `bin/jobs` against the primary DB, so these
# tables must exist on backend_development / backend_test.
